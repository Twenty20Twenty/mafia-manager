...
public interface TournamentParticipantRepository extends JpaRepository<TournamentParticipant, Long> {
    ...
    /** Все участия конкретного пользователя. */
    @Query("SELECT tp FROM TournamentParticipant tp WHERE tp.user.id = :userId AND tp.status = :status")
    List<TournamentParticipant> findByUserIdAndStatus(
            @Param("userId") Long userId,
            @Param("status") ParticipantStatus status
    );

    /** Все участники конкретной команды. */
